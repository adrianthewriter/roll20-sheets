 ...
 *
 * ```js
 * const sheet = new RunesmithSheetWorkers('awesome-game', '1.0.0')
 *
 * sheet
 *   .listen('sheet:opened')
 ...
 * ```
 */
export default class RunesmithSheetWorkers {
  constructor(gameId = name, ver = version) {
    this.__gameId = gameId
    this.__version = ver
    this.__context = new RunesmithContext()
  ...
  __toolbox() {
    return {
       ...
       * @param {String} sectionName - The name of a repeating table to narrow results.
       * @param {Function} func - A callback function to run after getAllFields() completes.
       */
      getAllFields: (fieldSet, sectionName, func) => {
        getAttrs([`_reporder_repeating_${sectionName}`], (v) => {
          getSectionIDs(sectionName, (idArray) => {
            let reporderArray = v[`_reporder_repeating_${sectionName}`]
                ? v[`_reporder_repeating_${sectionName}`]
                    .toLowerCase()
                    .split(',')
                : [],
              rowIds = [
            ...
            let fieldArray = Object.getOwnPropertyNames(
              fieldSet[sectionName][0]
            )

            let fieldsFromAllRows = new Set()
            rowIds.forEach((id) => {
              fieldArray.forEach((field) => {
                if (field !== 'rowId')
                  fieldsFromAllRows.add(
                    `repeating_${sectionName}_${id}_${field}`
                  )
              })
            })

            getAttrs(Array.from(fieldsFromAllRows), (values) => {
              const { fieldSet } = this.util.parseValues(values)
              _.partial(func, fieldSet[sectionName])()
            })
          })
        })
      },
      ...
      getAttrs(_.union(attrNames, fieldNames), (values) => {
        ...
        try {
          _.partial(func, {
            ...
            attrSet,
            fieldSet,
            getAllFields: _.partial(this.toolbox.getAllFields, fieldSet),
...
/**
 * @class
 * The RunesmithContext class used internally by RunesmithSheetWorkers.
 */
class RunesmithContext {
  constructor() {
    this.__triggers = []
    this.__attrs = []
    this.__fields = []
